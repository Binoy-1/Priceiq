...
def demand_vs_price(a: float, b: float, floor: float = 1.0,
                    ceiling: float = 20.0, height: int = 340,
                    actual_prices: np.ndarray | None = None,
                    actual_demands: np.ndarray | None = None,
    ...
    if actual_prices is not None and actual_demands is not None:
        valid = (actual_prices > 0) & (actual_demands > 0)
        if valid.any():
            fig.add_trace(go.Scatter(
                x=actual_prices[valid], y=actual_demands[valid],
                mode="markers", name="Actual data",
                marker=dict(color="rgba(31,29,26,0.35)", size=4, line=dict(width=0)),
                hovertemplate="price=$%{x:.2f}<br>demand=%{y:.0f}<extra></extra>",
            ))
    if optimal_price is not None and not np.isnan(optimal_price):
        fig.add_vline(x=optimal_price, line_dash="dot", line_color=BRAND,
                      line_width=2,
