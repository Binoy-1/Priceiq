...
def render() -> None:
    ...
    inv_factor = 1.0 + 0.2 * (1.0 - inventory / 5000)
    q_adjusted[2] *= inv_factor  # boost RAISE when inventory is low
    ...
    with right:
        ...
        st.plotly_chart(confidence_gauge(confidence), width='stretch')
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    # Actual data overlay (from uploaded dataset)
    actual_prices, actual_demands = None, None
    ds = st.session_state.get("active_dataset")
    roles = st.session_state.get("active_roles")
    if ds is not None and roles is not None:
        pc = roles.get("price")
        qc = roles.get("quantity")
        if pc and qc and pc in ds.columns and qc in ds.columns:
            actual_prices = ds[pc].astype(float).values
            actual_demands = ds[qc].astype(float).values
    optimal_price = st.session_state.get("forecast_optimal_price")

    # Scenario comparison
    st.markdown(section("Scenario Comparison", "Demand & revenue across the price range"), unsafe_allow_html=True)
    c7, c8 = st.columns(2)
    with c7:
        st.plotly_chart(demand_vs_price(a, b, actual_prices=actual_prices,
