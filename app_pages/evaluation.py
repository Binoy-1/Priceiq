...
def render() -> None:
    ...
    mean_r_revenue = mean_r * 1000
    lift = ((mean_r_revenue - fixed) / fixed * 100) if fixed else 0.0
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(reward_distribution(mean_r, std_r), width='stretch')
    with c2:
        st.plotly_chart(bar(["Learned policy", "Fixed-optimal"], [mean_r * 1000, fixed],
