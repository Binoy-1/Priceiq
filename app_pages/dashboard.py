...
def render() -> None:
    ...
    cards = [
        ...
        ("Mean Reward",        f"{mean_reward:.3f}", f"per step · ${mean_reward * 1000:,.0f} revenue equiv.", False, "✦", reward_spark),
    ...
    with c3:
        if coef is not None:
            ...
            st.plotly_chart(demand_vs_price(a, b), width='stretch')
    with c4:
        if "action" in recs:
            counts = recs["action"].value_counts().to_dict()
            st.plotly_chart(action_distribution(counts), width='stretch')
        else:
            st.plotly_chart(epsilon_curve(0.3, 0.01, 0.995, 1000), width='stretch')
