...
def _dataset_mode() -> None:
    ...
    try:
        with st.spinner("Running forecast — this may take a few seconds..."):
    ...
    if "price" in ts and "quantity" in ts:
        ...
        st.session_state["forecast_optimal_price"] = p_opt
        c1, c2 = st.columns(2)
        with c1:
            fig = go.Figure()
...
def _rl_mode() -> None:
    ...
    if qsum:
        ...
        st.plotly_chart(quarterly_price(qdf), width='stretch')
